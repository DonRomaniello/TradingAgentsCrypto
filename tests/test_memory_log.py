...
def _price_df(prices):
    """Minimal DataFrame matching ccxt get_ohlcv output shape."""
...
def _make_pm_state(past_context=""):
    ...
    return {
        ...
        "tokenomics_report": "Tokenomics report.",
...
class TestTradingMemoryLogCore:
    ...
    def test_get_pending_returns_pending_only(self, tmp_path):
        log = make_log(tmp_path)
        _seed_completed(tmp_path, "BTC/USDT", "2026-01-05", "Buy NVDA.", "Correct.")
        log.store_decision("BTC/USDT", "2026-01-10", DECISION_BUY)
    ...
    def test_get_past_context_same_ticker(self, tmp_path):
        log = make_log(tmp_path)
        _seed_completed(tmp_path, "BTC/USDT", "2026-01-05", "Buy BTC/USDT — network adoption thesis intact.", "Directionally correct.")
    ...
    def test_get_past_context_cross_ticker(self, tmp_path):
        log = make_log(tmp_path)
        _seed_completed(tmp_path, "ETH/USDT", "2026-01-05", "Buy ETH/USDT — DeFi TVL growth.", "Correct.")
        ctx = log.get_past_context("BTC/USDT")
    ...
    def test_n_cross_limit_respected(self, tmp_path):
        ...
        log = make_log(tmp_path)
        for i, ticker in enumerate(["ETH/USDT", "SOL/USDT", "XRP/USDT", "DOGE/USDT"]):
...
class TestDeferredReflection:
    ...
    def test_update_preserves_other_entries(self, tmp_path):
        ...
        assert len(entries) == 3
        btc, eth, sol = entries
        assert btc["ticker"] == "BTC/USDT" and btc["pending"] is True
        assert eth["ticker"] == "ETH/USDT" and eth["pending"] is False
        assert eth["reflection"] == "Neutral result."
        assert sol["ticker"] == "SOL/USDT" and sol["pending"] is True
    ...
    def test_fetch_returns_valid_ticker(self):
        ...
        mock_graph.config = {"reflection_benchmark": "BTC/USDT"}
        with patch("tradingagents.graph.trading_graph.get_ohlcv") as mock_ohlcv:
            mock_ohlcv.side_effect = lambda inst, tf, since, until, **kw: (
                _price_df(bench_prices) if inst.base == "BTC" else _price_df(stock_prices)
            )
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "SOL/USDT", "2026-01-05")
    ...
    def test_fetch_returns_too_recent(self):
        ...
        mock_graph.config = {"reflection_benchmark": "BTC/USDT"}
        with patch("tradingagents.graph.trading_graph.get_ohlcv") as mock_ohlcv:
            mock_ohlcv.return_value = _price_df([100.0])
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "SOL/USDT", "2026-04-19")
    ...
    def test_fetch_returns_delisted(self):
        ...
        mock_graph.config = {"reflection_benchmark": "BTC/USDT"}
        with patch("tradingagents.graph.trading_graph.get_ohlcv") as mock_ohlcv:
            mock_ohlcv.return_value = pd.DataFrame({"close": []})
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "SOL/USDT", "2026-01-10")
        assert raw is None and alpha is None and days is None

    def test_fetch_returns_benchmark_shorter_than_asset(self):
        """Benchmark having fewer rows than asset must not raise IndexError."""
        stock_prices = [100.0, 102.0, 104.0, 103.0, 105.0, 106.0]
...
class TestPortfolioManagerInjection:
    ...
    def test_n_cross_limit_respected(self, tmp_path):
        ...
        log = make_log(tmp_path)
        tickers = ["ETH/USDT", "SOL/USDT", "BNB/USDT", "ADA/USDT", "XRP/USDT"]
